# Authors and Citation

## Authors

- **[Yen-Chieh Liao](https://davidycliao.github.io)**. Author,
  maintainer.

- **[Li Tang](https://sites.google.com/view/litang2020?pli=1)**.
  Contributor, author.

- **OpenFun Ltd.**. Funder, contributor, copyright holder.

- **Shaka Y.J. Li**. Contributor.

- **Taiwan Legislative Yuan**. Copyright holder, funder.

- **Taiwan National Science and Technology Council**. Contributor.

- **Taiwan Science & Technology Policy Research and Information Center
  NARLabs**. Contributor.

## Citation

Source:
[`DESCRIPTION`](https://github.com/davidycliao/legisTaiwan/blob/HEAD/DESCRIPTION)

Liao Y, Tang L (2026). *legisTaiwan: An Interface to Access Taiwan
Legislative API in R*. R package version 0.2.2,
<https://davidycliao.github.io/legisTaiwan/>.

    @Manual{,
      title = {legisTaiwan: An Interface to Access Taiwan Legislative API in R},
      author = {Yen-Chieh Liao and Li Tang},
      year = {2026},
      note = {R package version 0.2.2},
      url = {https://davidycliao.github.io/legisTaiwan/},
    }
